...
#### 独立渲染配置 (JSON)
您可以直接在翻译的JSON文件中为每个文本区域（region）单独指定渲染参数，以实现精细的手动调整。渲染器会优先使用这里面的值。

**示例:**
```json
{
    "text": "这个字要大一点！",
    "translation": "This text should be bigger!",
    "lines": [...],
    "render_opts": {
        "font_size": 42,
        "font_color": "FF0000:000000", 
        "alignment": "center",
        "direction": "h"
    }
}
...
**可覆盖的参数:**
*   `font_size` (整数)
*   `font_color` (字符串, e.g., "RRGGBB:AABBGGRR")
*   `alignment` (字符串: "left", "center", "right")
*   `direction` (字符串: "h" or "v")
*   `line_spacing` (浮点数)
*   `disable_font_border` (布尔值)

#### 渲染参数
```
renderer          渲染从漫画翻译的文本，并进行额外的排版处理。会忽略某些其他参数选项
force_strict_layout 强制渲染器严格遵守文本框边界，禁止文本溢出。默认为false
alignment         对齐渲染的文本
